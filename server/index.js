...
app.post("/deploy/:network", async (req, res) => {
  ...
  if (!signedTransactionJSON) {
    return res
      .status(400)
      .json({ error: "Missing signedTransactionJSON in body" });
  }
  ...
  try {
    ...
    return res
      .status(400)
      .json({ error: "Bad signedTransactionJSON", details: e.message });
  }
  ...
  try {
    ...
    console.error(
      "putTransaction error:",
      e?.response?.data || e?.data || e?.message || e,
    );
    res.status(502).json({
